...
const io = new Server(httpServer, {
  ...
  cors: {
    origin: true,
    credentials: true,
    allowedHeaders: ['x-acarreosya-auth-token', 'authorization', 'content-type'],
    methods: ['GET', 'POST'],
  },
...
io.use((socket, next) => {
  try {
    ...
    const queryToken = typeof socket.handshake.query?.authToken === 'string'
      ? socket.handshake.query.authToken
      : undefined;

    const token = ((authToken || headerToken || bearerToken || queryToken || '') + '').trim();
    ...
    const userId = 43; // demo para pruebas locales
...
io.on('connection', async (socket) => {
  ...
  socket.on('driver:location', async (payload, cb) => {
    try {
      ...
      if (!vehicleId || !driverId || typeof lat !== 'number' || typeof lon !== 'number') {
      ...
      io.to(room).emit('server:broadcast', { vehicleId, lat, lon, speed, heading, sentAt });
      io.emit('driver:location:ack', { vehicleId, lat, lon, speed, heading, sentAt });
